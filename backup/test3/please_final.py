...
def add_to_current_chunk_vectorstore(chunks: List[str], episode_seq: int = DEFAULT_EPISODE_SEQ):
    ...
    metadatas = [
        { "novel_id": CURRENT_NOVEL_ID, "chunk_index": i, "source_seq": str(episode_seq)}
...
def extract_facts_from_chunk(chunk: str, chunk_type: str) -> str:
    ...
    system_msg = (
        ...
        "category: PHY_STATUS/PHY_TRAIT/ABILITY/ITEM/RELATION/LOCATION/WORLD_SETTING/EMOTION\n"
        ...
        "\n"
        "TYPE_A(설정/세계관): 가능한 한 모든 사실/규칙을 세분화해서 FACT/RULE/EXCEPTION으로 추출해.\n"
        "TYPE_B(감정/내면): 앞으로 전개나 관계에 영향을 줄 만큼 강한 '지속적인 감정 상태'나 '결심'만 FACT로 추출해.\n"
        ...
        "\n"
        "[FACT 추출시 강력 규칙 – 오염 방지]\n"
        "1. '사실 여부가 0/1로 판정 가능한 정보'만 FACT로 저장한다.\n"
        "   - 생사, 위치, 시간, 관계, 숫자, 외형, 물리적 상태만 포함.\n"
        "2. 감정, 심리, 태도, 욕설, 비유, 분위기, 평가, 캐릭터 성향 등은 FACT로 저장하지 않는다.\n"
        "3. '~할 수도 있다', '~일 것이다', '~인 듯하다'처럼 모호한 표현은 FACT로 간주하지 않는다.\n"
        "4. 인물 대사나 욕설 속 과장된 표현은 FACT로 저장 금지이다.\n"
        "   (예: '조밥도 못 먹는 년' → 욕설이지 건강 상태 FACT가 아님)\n"
        "5. 기존 FACT와 충돌 여부를 판단하기 전에, 해당 문장이 '객관적 FACT'인지 먼저 검사하고,\n"
        "   FACT가 아니면 DB에 저장하지 않는다.\n"
        "6. 대화를 FACT로 저장할 때는, 반드시 대화문임을 명확히 인지하고,화자의 상태를 확인해야 한다.\n"
        "  (예: '나는 지금 화가 났다'는 감정 상태이지 FACT가 아님)\n"
        "7. 대화문에서 화자가 술에 취해 있거나 거짓말을 하는 경우, 그 내용은 FACT로 저장하지 않는다.\n"
        "8. FACT로 추출한 내용은 반드시 원문에 명확히 드러나 있어야 한다.\n"
        "9. 절대 추론이나 상상을 통해 FACT를 생성하지 않는다.\n"
    )
...
def save_to_current_db(chunk: str, facts_json: str) -> str:
    ...
    try:
        ...
        normalized = [
            _build_lore_item(fact, chunk, source_seq=CURRENT_EPISODE_SEQ, novel_id=CURRENT_NOVEL_ID)
            for fact in facts
        ]
...
def create_lore_keeper_agent(model_name: str = "gpt-4o-mini"):
    ...
    workflow = (
        ...
        "   e. (필요할 경우) search_full_story_db (이전 회차 원문 맥락 검색)\n"
...
**[충돌 판정 매트릭스 (Strict Guidelines)]**

**1. HARD CONFLICT (물리적/팩트 모순)**
- **핵심 철학:** "일어나지 않은 일은 모순이 아니다. **일어날 수 없는 일**만 모순이다."
- **판정 기준:** 반드시 **DB에 존재하는 명시적 증거**가 있어야 한다. 증거 없이는 절대 Hard Conflict를 낼 수 없다.
- 예를 들어, 시간적 모순, 상태 모순, 규칙 위반 등이 이에 해당한다.
  - (O) DB: "A는 어제 서울에 있었다" vs Input: "A는 오늘 뉴욕에 있었다" -> **충돌** (시간적 모순)
  - (O) DB: "A는 불사신이다" vs Input: "A는 죽었다" -> **충돌** (불사신과 사망은 양립 불가)
  - (O) DB: "A는 5분 전에 사망했다" vs Input: "A가 밥을 먹는다" -> **충돌** (사망 상태와 식사는 양립 불가)
  - (X) DB: (A의 위치 정보 없음) vs Input: "A는 감옥에 있다" -> **PASS** (기존 정보가 없으므로 새로운 사실임)
  - (X) DB: "A는 감옥에 있다" vs Input: "A는 감옥에 있다" -> **PASS** (재확인)
- **환각 금지:** "이전 회차에서 탈출했다고 했다" 같은 없는 사실을 지어내지 마라. DB 검색 결과에 없으면 없는 것이다. 이 점을 유의하라.

============================================================
📌 Soft Conflict — 극도로 명확한 ‘사실 기반 모순’만 판정한다
============================================================

Soft Conflict는 다음 조건을 **모두** 충족할 때만 발생한다:

1) Lore_DB 또는 Current_DB에 **사실(FACT)로 단정된 정보**가 있어야 한다.
   - “죽었다”, “없다”, “부산에 있다”, “아이 없다”, “절대 용서하지 않는다” 등
   - 반드시 ‘0/1’로 판정 가능한 팩트여야 한다.

2) 현재 청크가 그 ‘단정된 사실’과 **정면으로 모순되는 명백한 반대 사실**을 말해야 한다.
   - 독자가 100% “이건 논리적으로 불가능하다”라고 느낄 정도여야 함.

3) **추론/해석·개연성·느낌 기반 판단은 절대 금지**.
   - 인간 감정, 이상한 행동, 기괴한 반응, 심리적 부조화는 Soft Conflict가 아님.

============================================================
❌ 아래 항목들은 절대로 Soft Conflict가 아니다 (오탐 방지 규칙)
============================================================

- 감정 변화, 감정의 폭발, 감정 기복
- 상실 후 이상 행동(웃음, 춤, 무감정 등)
- 병이 있는데 먹는 행동 / 가난한데 설렁탕 먹고 싶어함
- 슬픈데 웃음 / 기쁜데 눈물 / 혼란스러운 행동
- 비유, 은유, 상징적 표현
- 이해하기 힘든 행동, 비논리적인 선택
- LLM이 추론한 '캐릭터성' 또는 '성격 일관성'
- “개연성이 부족하다”, “이해가 잘 안 된다” 수준의 감정적 불일치

👉 **즉, 감정·행동·반응은 어떤 것이든 Soft Conflict가 될 수 없다.**  
   *오직 “논리적 사실”이 모순될 때만 Soft Conflict.*

============================================================
✔ Soft Conflict로 인정되는 사례 (진짜 충돌만)
============================================================

(1) 생사/존재 모순
    - FACT: “아내는 죽었다”
    - 현재: “아내가 걸어왔다”, “밥을 먹었다”, “대화를 나눴다”
...
Soft Conflict 조건 =  
“**단정된 FACT**가 있고 → **그 FACT의 완전 반대가 즉시 등장**할 때만 발생.”

감정·행동·심리 묘사·비논리적 선택·기이한 행동은 **모두 PASS**.

============================================================


**3. PASS (상태 변화 및 전개 - 저장)**
- **상태 업데이트:** 위치 이동, 신분 상승, 감정 변화는 모순이 아니라 **스토리 진행**이다.
- **새로운 사건:** DB에 없는 새로운 인물, 사건, 설정은 무조건 수용해라.
...
**충돌이 아닌 경우(다음 상황에서는 report_conflict_to_db를 절대 호출하지 마):**
- 작품에서 처음 등장하는 인물/아이템/장소/사건건 등이 소개되는 경우.
...
- 이 경우 search_* 도구를 호출하지 않고 바로 save_to_current_db를 호출.
...
def process_chunk_with_agent(agent_graph, chunk: str, index: int) -> Dict[str, Any]:
    ...
    try:
        ...
        chunk_type = newly_saved[0].get("chunk_type", "TYPE_A") if newly_saved else "TYPE_D"
