...
def extract_facts_from_chunk(chunk: str, chunk_type: str) -> str:
    ...
    system_msg = (
        ...
        "- **한 번 일어난 중요한 사건(예: A가 B를 죽였다, 결혼했다, 전쟁이 났다, 해고되었다 등)도 FACT로 추출해.\n" 
        "- 이런 건 category를 EVENT로 두고, subject는 행위 주체, effect에는 간단한 사건 내용을 쓴다.**\n"
        "TYPE_B(감정/내면): 앞으로 전개나 관계에 영향을 줄 만큼 강한 '지속적인 감정 상태'나 '결심'을 우선 FACT로 추출해.\n"
        ...
        "   정말 애매하면, 문장의 핵심 내용을 한 줄로 요약한 FACT나 EMOTION을 1개 만들어.\n"
        ...
        "   - 같은 chunk 안에서 같은 내용에 대해 스스로 부정하거나 단정짓는 표현이 나오면, 단정하는 부분만 FACT로 추출해라."
        "   (예: '아니다', '결코 아니다'처럼 스스로 부정하는 표현 -> '가기 싫어서가 아니다'처럼 단정하는 쪽만 FACT)\n"
        "5. 인물 대사나 욕설 속 과장된 표현, 반어법은 FACT로 추출하지 않는다.\n"
        "   (예: '조밥도 못 먹는 년' → 욕설이지 건강 상태 FACT가 아님)\n"
        "6. 대화를 FACT로 저장할 때는, 반드시 대화문임을 명확히 인지하고, 화자와 청자의 상태를 확인해야 한다.\n"
        "   - 화자와 청자는 명확하게 인물의 이름(그, 그녀와 같은 인칭 대명사 불가)을 명시한다. 만약 이름을 찾을 수 없으면 사실로 추출하지 않는다.\n"
        "7. 대화문에서 화자가 술에 취해 있거나 거짓말을 하는 경우, 그 내용은 FACT가 아니다.\n"
        "   - 단, 청자에게는 FACT일 수 있다. (예: 그가 사람을 죽였다는 것을 K가 들었다 ->  K는 그가 사람을 죽였다고 들음)\n"
        ...
        "9. FACT로 추출한 내용은 반드시 원문에 명확히 드러나 있어야 한다.\n"
        "10. 절대 원문에 없는 새로운 설정을 상상해서 FACT를 생성하지 않는다.\n"
        "\n"
        "[item_type 의미]\n"
        "- FACT: 지금 장면에서 실제로 벌어진 상태/사건 (시간·공간·상태가 구체적).\n"
        "- RULE: 세계/사회/마법/직업/인물의 행동 패턴에 대한 '반복되는 일반 규칙'.\n"
        "- EXCEPTION: 기존 RULE을 깨는 '이 경우만 예외' 설정.\n"
        "- 개인 신조/습관/미신(예: 그는 병이란 놈에게 약을 주면 더 심해진다고 믿는다)은 item_type=\"RULE\", target_group=\"INDIVIDUAL\"로 추출해라."
...
감정·행동·심리 묘사·비논리적 선택·기이한 행동·타인의 행동에 의한 연계 반응은 **모두 PASS**.
...
- 실제로 행동한 것이 아니라 단순히 서술상에서 '~했다면', '~하면'과 같이 가정을 하는 경우
    ...
    system_msg = (
        "너는 '설정 충돌 전용 판정기'야. "
        "주어진 FACT들과 아래에 주어지는 EvidenceContext 문자열 안에 들어 있는 내용만 근거로 " 
        "Hard/Soft Conflict 여부를 판정해.\n"
        "- EvidenceContext에 없는 정보(원작 줄거리, 네 세계 지식, 상식)는 절대 쓰면 안 된다.\n"
        "- Evidence가 없으면 무조건 is_conflict=false로 해라.\n"
        "- is_conflict=true일 때, conflicting_text의 source_seq를 source_seq 값으로 해라."
        "FACT의 text 의미를 비유, 반어법, 사건의 시간적 배경(그때, 예전과 같은 과거 표현, 현재가 아니라는 증거) 등을 고려해 정확히 파악해라. 절대 문장의 일부만 추출하여 충돌을 판단하지 마라."
        "\n"
    ...
    user_msg = (
        ...
        f"[FACTS_JSON]\n{facts_json}\n\n"
        f"[EVIDENCE_CONTEXT]\n{evidence_context}\n"
        "위 정보를 바탕으로 설정 충돌 여부를 판정하고, 다음 JSON 형식으로만 답해:\n"
        "{"
        "\"is_conflict\": true/false, "
        "\"conflict_type\": \"Hard Conflict\" 또는 \"Soft Conflict\" 또는 \"\", "
        "\"reason\": \"...\", "
        "\"conflicting_text\": \"...\""
        "}"
    )
...
def create_lore_keeper_agent(model_name: str = "gpt-4o-mini"):
    ...
    if CURRENT_EPISODE_SEQ == 1:
        ...
        workflow = (
            ...
            "   f. (필요할 경우) search_full_story_db (이전 회차 원문 맥락 검색)\n"
            ...
            "*c, d를 통해 찾은 설정이 현재의 문장 충돌을 검사하기에 충분하지 않으면 e, f를 호출한다.\n"
            "   - 이때, 충분하지 않다는 것은 찾은 설정들이 현재의 문장과 관련이 없거나 동떨어진 정보일 경우, 판단을 하기에 정보가 부족한 경우를 말한다.\n"
            "   (예 : 개똥이의 나이에 대한 정보가 없는 경우, 설정과 관련 없는 특정 사건이 제시된 경우)\n"
        )
